...
import Modal, { FormGroup, FormRow } from './Modal'
import { fetchStockPrice, fetchStockPrices, isRemoteReadonly } from '../api'

const won = n => `${Math.round(Number(n) || 0).toLocaleString('ko-KR')}원`
const today = () => new Date().toISOString().slice(0, 10)
const BLANK_HOLDING = { ticker: '', name: '', shares: '', price: '', note: '' }

export default function IsaAccountPage({
  title,
  subtitle,
  syncSourceLabel,
  syncErrorLabel,
  ...
  const load = () => {
    getHistory().then(setList).catch(() => {})
    getHoldings().then(setHoldings).catch(() => {})
  }

  useEffect(() => { load() }, [getHistory, getHoldings])
  ...
  const handleTickerLookup = async () => {
    ...
    try {
      ...
      alert(e.response?.data?.detail || '티커 조회에 실패했습니다.')
    } finally {
  ...
  const syncBalance = async () => {
    ...
    try {
      ...
      await Promise.all(updated.map(h => updateHolding(h.id, h)))
      const total = updated.reduce((sum, h) => sum + (Number(h.shares) || 0) * (Number(h.price) || 0), 0)
      await createHistory({ date: today(), value: Math.round(total), note: 'Yahoo 현재가 기준 ETF 자동 동기화' })
      setLastUpdated(new Date().toLocaleTimeString('ko-KR'))
      load()
    } catch {
      alert('총액 자동 동기화에 실패했습니다.')
    } finally {
  ...
  const syncBroker = async () => {
    ...
    try {
      const data = await syncFromBroker()
      setLastUpdated(new Date().toLocaleTimeString('ko-KR'))
      load()
      alert(`${syncSourceLabel} 동기화 완료: ${data.count}개 종목, ${won(data.value)}`)
    } catch (e) {
      alert(e.response?.data?.detail || syncErrorLabel)
    } finally {
  ...
  const submitBalance = async () => {
    ...
    try {
      await createHistory({ ...form, value: +form.value })
  ...
  const removeBalance = async (id) => {
    if (!confirm('이 총액 기록을 삭제할까요?')) return
    ...
    <div className="page-enter">
      <div className="page-header">
        ...
        {!isRemoteReadonly && (
          <div className="page-header-actions">
            <button className="btn btn-primary" onClick={syncBroker} disabled={brokerLoading}>
              {brokerLoading ? `${syncSourceLabel} 동기화 중...` : `${syncSourceLabel}에서 가져오기`}
            </button>
            <button className="btn btn-ghost" onClick={handleBulkPriceUpdate} disabled={priceLoading || !holdings.length}>
              {priceLoading ? '업데이트 중...' : '가격 업데이트'}
            </button>
            <button className="btn btn-ghost" onClick={syncBalance} disabled={syncLoading || !holdings.length}>
              {syncLoading ? '동기화 중...' : '총액 자동 동기화'}
            </button>
            <button className="btn btn-ghost" onClick={() => openBalanceModal()}>수동 총액 입력</button>
      ...
      <div className="grid-3">
        ...
        <div className="stat-card accent-green">
          <div className="stat-label">기록된 ISA 총액</div>
        ...
        <div className="stat-card accent-gray">
          ...
          <div className="stat-sub">총액 기록 {list.length}건</div>
        </div>
      </div>

      <div className="section">
        <div className="section-header">
          <div className="section-title">{holdingsLabel}</div>
        </div>
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>티커</th><th>종목명</th><th className="num">보유 수량</th>
              <th className="num">현재가</th><th className="num">평가금액</th><th>메모</th><th></th>
            </tr></thead>
            <tbody>
              {holdings.length === 0 ? (
                <tr><td colSpan={7}>
                  <div className="empty"><div className="empty-icon">ETF</div><p>등록된 종목이 없습니다.</p></div>
                </td></tr>
              ) : holdings.map(h => {
      ...
      <div className="section">
        <div className="section-header">
          <div className="section-title">총액 이력</div>
        </div>
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>기록일</th><th className="num">총액</th><th className="num">증감</th><th>메모</th><th></th>
            </tr></thead>
            <tbody>
              {list.length === 0 ? (
                <tr><td colSpan={5}>
                  <div className="empty"><div className="empty-icon">Σ</div><p>기록된 총액이 없습니다.</p></div>
                </td></tr>
        ...
        <FormGroup label="티커">
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={handleTickerLookup} disabled={tickerLoading || !holdingForm.ticker.trim()}>
              {tickerLoading ? '조회 중...' : '조회'}
            </button>
          </div>
        </FormGroup>
        <FormGroup label="종목명">
          <input className="form-input" value={holdingForm.name} onChange={e => setHolding('name', e.target.value)} placeholder="조회 시 자동 입력" />
        </FormGroup>
        <FormRow>
          ...
          <FormGroup label="현재가">
            <input className="form-input" type="number" step="1" value={holdingForm.price} onChange={e => setHolding('price', e.target.value)} placeholder="0" />
          </FormGroup>
        </FormRow>
        <FormGroup label="메모">
      ...
      <Modal open={balanceModal} onClose={() => setBalanceModal(false)} title="ISA 총액 업데이트">
        <FormRow>
          <FormGroup label="기록일">
